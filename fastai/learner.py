...
# %% ../nbs/13a_learner.ipynb 3
from __future__ import annotations
from .data.all import *
from .optimizer import *
from .callback.core import *
from contextlib import nullcontext
import cloudpickle,threading
...
def load_learner(fname, cpu=True, pickle_module=pickle, backward_compat=False):
    ...
    try:
        ...
        if not any(m in str(e) for m in ("fastcore.transform", "fastcore.dispatch")): raise e
        if not backward_compat: raise RuntimeError("Your Learner was most likely trained under `fastai<2.8`, you can load such Learners by setting load_learner(...,backward_compat=True). But you should retrain it under `fastai>=2.8` for continued compatibility. This compatibility layer will be removed in `fastai>=2.9`")
        warn(category=DeprecationWarning,message="Your Learner was most likely trained under `fastai<2.8. You should retrain it under `fastai>=2.8` for continued compatibility. This compatibility layer will be removed in `fastai>=2.9`")
        import sys
        import fastai._compat.fastcore_dispatch
        import fastai._compat.fastcore_transform
        sys.modules['fastcore.dispatch'] = fastai._compat.fastcore_dispatch
        sys.modules['fastcore.transform'] = fastai._compat.fastcore_transform
        res = torch.load(fname, map_location=map_loc, pickle_module=pickle_module, **load_kwargs)
        _ = sys.modules.pop("fastcore.dispatch")
        _ = sys.modules.pop("fastcore.transform")
